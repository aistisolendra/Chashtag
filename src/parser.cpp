...
{
    bool isDoubleIf = false;
    auto lvalue = ParsePostUnary(ParsePreUnary());
    if (!lvalue)
        return nullptr;
    return ParseBinOP(0, move(lvalue), isDoubleIf);
}
...
unique_ptr<Expression> Parser::ParseBinOP(int minPrec, unique_ptr<Expression> lvalue, bool isDoubleIf)
{
   while (true)
    {
        ...
        auto rtype = rvalue->getType();
         ...
         if (lexer.getCurrentToken() == -21 || isDoubleIf || lexer.getCurrentToken() == -22 || lexer.getCurrentToken() == -25
         || lexer.getCurrentToken() == -24 || lexer.getCurrentToken() == -23 || lexer.getCurrentToken() == -30)
         {
            isDoubleIf = true;

            if (prevPrec < getOperatorPrecedence())
            {
                rvalue = ParseBinOP(prevPrec + 1, move(rvalue), isDoubleIf);
                if (!rvalue)
                    return nullptr;
            }
            ...
            while(true)
            {
                int prevPrec2 = getOperatorPrecedence();

                if (prevPrec2 < minPrec)
                    return move(lvalue);

                auto BinOp2 = returnBinOpsType();
                lexer.getNextToken();
                auto rrvalue = ParsePostUnary(ParsePreUnary());
                if (!rrvalue)
                    return nullptr;
                auto rrtype = rrvalue->getType();

                if (!ltype->doesMatch(rtype) || !rtype->doesMatch(rrtype))
                {
                    LogError("Type Error");
                    return nullptr;
                }
                if (!BinOp->validOperandSet(ltype) || !BinOp2->validOperandSet(rtype))
                {
                    LogError("Type of Operands given to Operator is Illeagal");
                    return nullptr;
                }

                if (prevPrec2 < getOperatorPrecedence())
                {
                    rrvalue = ParseBinOP(prevPrec2 + 1, move(rrvalue), isDoubleIf);
                    if (!rrvalue)
                        return nullptr;
                }
                  auto OpType = BinOp->getOperatorEvalTy();
                  auto OpType2 = BinOp2->getOperatorEvalTy();
                
                  lvalue = make_unique<BinaryExpressionTwo>(move(BinOp), move(BinOp2), move(lvalue), move(rvalue), move(rrvalue), move(OpType));
            }
       }
        else
        {
...
unique_ptr<FunctionSignature> Parser::ParseDefine()
{
    lexer.getNextToken();
    ...
    if (!lexer.isTokenSemiColon())
    {
        LogError("Expected a ';' after 'define'");
        return nullptr;
    }
    LocalVarTable.clear();
    return fs;

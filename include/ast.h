...
namespace AST
{
    ...
    class BinaryExpressionTwo : public Expression
    {
        unique_ptr<BinOps> op;
        unique_ptr<BinOps> op2;
        unique_ptr<Expression> LVAL;
        unique_ptr<Expression> MVAL;
        unique_ptr<Expression> RVAL;

    public:
        BinaryExpressionTwo(unique_ptr<BinOps> op, unique_ptr<BinOps> op2, unique_ptr<Expression> LVAL, unique_ptr<Expression> MVAL, unique_ptr<Expression> RVAL, unique_ptr<::Type> ExpressionType) : op(move(op)), op2(move(op2)), LVAL(move(LVAL)), MVAL(move(MVAL)), RVAL(move(RVAL)), Expression(move(ExpressionType))
        {
        }
        llvm::Value *codeGen() override;
        void VarDecCodeGen(GlobalVariable *, ::Type *) override;
